...
class PrintPrescriptionPage extends StatefulWidget {
  final List<Prescription> listPrescription;

  const PrintPrescriptionPage({Key key, @required this.listPrescription}) : super(key: key);
  @override
  _PrintPrescriptionState createState() => _PrintPrescriptionState();
}

class _PrintPrescriptionState extends State<PrintPrescriptionPage> {
    ...
    return Scaffold(
      ...
      body: PdfPreview(
        ...
        build: (PdfPageFormat format) async {
          return await generateInvoice(pageFormat: format, list: widget.listPrescription);
        },
      ),
...
FutureOr<Uint8List> generateInvoice({PdfPageFormat pageFormat, List<Prescription> list}) async {
  ...
  pw.Widget _buildHeader(pw.Context context) {
    return pw.Column(
                children: [
                  pw.Container(
                    height: 50,
                    padding: const pw.EdgeInsets.only(left: 20, bottom: 48,),
    ...
    return pw.Table.fromTextArray(
      ...
      data: List<List<String>>.generate(
        prescriptions.length,
          ...
          tableHeaders.length,
              (col) => prescriptions[row].getIndex(col),
        ),
      ),
    );
  }
